...
public class MapGenerator {
	...
	public boolean checkDiagonals(int direction, Location loc) { // loc here is location where generator wants to go
		...
		if (direction == MOVELEFT || direction == MOVERIGHT) {
			if (loc.getY() > 0) {
				if (loc.getY() < width-1) {
					if((canMove(MOVEUP, loc) && canMove(MOVEDOWN, loc))) {
						if(loc.getX() > 0 && loc.getX() < length-1) {
							System.out.println("checking if i'm not crossing with existing path on next tile " + canMove(direction, loc));
							return canMove(direction, loc); // check if not crossing with other path
						} else {
							return true;
						}
					}
					...
					return (canMove(MOVEUP, loc)); // needs same check for the next tile
				}
			} else {
				return canMove(MOVEDOWN, loc);
			}
		} else if (direction == MOVEUP || direction == MOVEDOWN) {
			if (loc.getX() > 0) {
				if (loc.getX() < length-1) {
					if((canMove(MOVERIGHT, loc) && canMove(MOVELEFT, loc))) {
						if(loc.getY() > 0 && loc.getY() < width-1) { // if we aren't on the edge then we can check that we are not crossing with other path
							System.out.println("checking if i'm not crossing with existing path on next tile " + canMove(direction, loc));
							return canMove(direction, loc);
						} else {
							return true;
						}
					}
					...
					return (canMove(MOVELEFT, loc));
				}
			} else {
				return canMove(MOVERIGHT, loc);
			}
		}
		return false;
	}
